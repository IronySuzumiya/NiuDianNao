...
std::string generate_classify_layer_code (unsigned num_input_neurons,
    ...
    do {
      ...
      remaining_input_neurons  = num_input_neurons;//8192
      num_nbout_to_write = std::min(remaining_output_neurons, num_output_neurons_per_entry * num_nbout_entries);//256

      unsigned total_num_nbin_entry = div_roundup(num_input_neurons, num_input_neurons_per_entry);//512
      unsigned num_input_to_load = std::min(remaining_input_neurons, num_input_neurons_per_entry * num_nbin_entries);//1024
      ...
      for (int current_nbin_entry = 0;
           current_nbin_entry < total_num_nbin_entry; current_nbin_entry = current_nbin_entry + 4) {
        ...
        // unsigned num_sb_to_load = num_input_neurons_per_entry * num_nbout_to_write;//4096
        unsigned num_sb_to_load = num_input_neurons_per_entry * num_nbout_to_write * 4;
        // load SB buffer
        ...
        if (nbin_entry_loaded == 0) { 
          num_input_to_load = std::min(remaining_input_neurons, num_input_neurons_per_entry * num_nbin_entries);//1024
          nbin_entry_loaded = div_roundup(num_input_to_load, num_input_neurons_per_entry);//64
          ...
          read_nbin(inst, 0, true, word_size);
        }

        // update counters
        nbin_entry_loaded -= 4;
        current_nbin_pointer += num_input_neurons_per_entry * word_size * 4;//16 * 2 * 4 
        cycles = std::max(cycles,
            div_roundup(inst.nbin_size / word_size, num_input_neurons_per_entry));
        
        // if(current_nbin_entry < 64){

        // }

        // prepare for output if it is last entry to finalize the sum
        if (current_nbin_entry == total_num_nbin_entry - 4) {
          current_nbout_pointer += current_sb_pointer * word_size;
          ...
          if (current_nbin_entry < 64) {
            partial_sum_NFU(inst, cp_inst::RESET);
            nop_NBout(inst);
          } else {
            partial_sum_NFU(inst, cp_inst::NBOUT);
            nop_NBout(inst);
          }
        }
